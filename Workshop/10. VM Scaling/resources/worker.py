...
fieldnames += "\n"
append_blob_service.append_blob_from_text(
    'distances', str(start) + "-" + str(stop) + '.csv', fieldnames)
...
def appendBlob(rowStr):
    while True:
        try:
            append_blob_service.append_blob_from_text(
                'distances', str(start) + "-" + str(stop) + '.csv', rowStr)
            break
        except:
...
for code1 in LatLongDict:
    ...
    if(rowIdx >= start and rowIdx <= stop):
        ...
        # outputs progress.
        if count % 10 == 0:
            print(str(start) + "-" + str(stop) + ": Processing " +
                  str(count) + " of " + str(stop - start) + " airports.")
...
append_blob_service.append_blob_from_text('distances', 'log.txt', "Finishing " + str(start) + "-" + str(
    stop) + ":" + datetime.datetime.fromtimestamp(time.time()).strftime('%Y-%m-%d %H:%M:%S') + "\n")
